...
论文运行路由见各自 `references/platform-compatibility.md`；其余技能见各自 `references/runtime.md`。不要求使用所有七个技能，只安装需要的完整目录即可。检索/下载 1.3.1、解析 1.4.0 与论文公众号 1.2.0 共享 paper-workspace/1 交接约定，也接受旧版 Markdown 笔记、PDF 与下载元数据。解析和论文公众号各自携带独立的资料/取图/网站归档脚本，只有其中一个也可运行。
