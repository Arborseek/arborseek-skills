...
| `html-to-office` | HTML 转 Word 与 PDF |
...
HTML 转换另需 lxml、Pillow、python-docx、Pandoc 3+；PDF 需 LibreOffice/soffice、pypdf 和可见中文字体。自带流程输出文档排版 PDF，不承诺还原任意网页 CSS。HTML 与资源须已在本地；不自动联网取图、执行 JS 或上传内容。可在 DeepSeek Harness 等支持标准 SKILL.md 且具有本地命令权限的宿主使用，未逐客户端测试。
