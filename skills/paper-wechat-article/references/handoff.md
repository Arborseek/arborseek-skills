...
读者图号由渲染器按最终正文配图顺序连续分配，与 paper_figure.label（原论文图号）分开；渲染报告 figure_numbers 保存 visual ID → 文章图号/原图号映射，不回写源档案。封面、公式图不占图号，rejected/未插入图不占号。正文引用请写 `<span data-figure-ref="f1">图</span>`，f1 对应 visuals.items[].id，渲染时同步改为实际“图 N”；引用未采用的图会报错。不要用全局文本替换改研究数据或“原论文图 5”等原文定位。旧稿里写死的“见图 N”需助手依据映射逐一更新，不能只改图注。
