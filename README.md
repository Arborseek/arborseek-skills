...
论文研究、内容创作、网站设计与办公效率，九个可独立使用的 AI 技能。由 Arborseek（天树探界）整理维护，显示名称以功能关键词为主。
...
合集版本：**1.11.0**。录屏教程与短视频脚本 **1.0.0**，检索/下载 **1.3.1**，解析 **1.4.0**，论文公众号 **1.4.1**，小红书 **3.2.1**，通用公众号 **1.1.2**，网站 **1.1.0**；技能独立版本以各自 `SKILL.md` 为准，目录与打包入口由 [catalog.json](catalog.json) 管理。保留既有 Slug，避免打断调用和交接。
...
2. 运行 `python3 scripts/package.py`，在 `dist/` 中获取合集包、九个普通独立 ZIP 和九个 `-workbuddy.zip` 根目录直接导入包。WorkBuddy 优先使用对应的直接导入包。
...
检查入口覆盖论文脚本回归、公众号原有测试、论文公众号交接与证据/配图门槛、小红书计划边界测试、短视频分镜导出、录屏提帧与教程导出及九技能结构/引用和目录搬迁检查。录屏测试使用模拟素材，不等于真实业务流程识别验收；缺可选依赖时会明确跳过相应测试。**未逐个完成五个客户端的原生导入与模型端到端测试**；Windows/Linux 原生运行也未实测。网络限流、图像生成、调度、OCR 与附件展示取决于实际环境，不能由技能格式兼容推断全部可用。
