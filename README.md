...
论文研究、内容创作、网站设计与办公效率，十个可独立使用的 AI 技能。由 Arborseek（天树探界）整理维护，显示名称以功能关键词为主。
...
| [短视频脚本与分镜助手](skills/short-video-storyboard/SKILL.md) | 八类企业短视频脚本、口播、分镜与拍摄清单，导出 CSV 和粗剪 SRT | `short-video-storyboard` |
| [录屏转操作教程](skills/screen-recording-to-guide/SKILL.md) | 录屏提帧、步骤核对、真实截图标注，导出图文手册与独立核对单 | `screen-recording-to-guide` |
| [HTML 转 Word 与 PDF](skills/html-to-office/SKILL.md) | 本地 HTML 单个/批量转可编辑 Word 与文档版 PDF，检查图片、表格、链接和分页 | `html-to-office` |

合集版本：**1.12.0**。HTML 转换、录屏教程与短视频脚本 **1.0.0**，检索/下载 **1.3.1**，解析 **1.4.0**，论文公众号 **1.4.1**，小红书 **3.2.1**，通用公众号 **1.1.2**，网站 **1.1.0**；技能独立版本以各自 `SKILL.md` 为准，目录与打包入口由 [catalog.json](catalog.json) 管理。保留既有 Slug，避免打断调用和交接。
...
2. 运行 `python3 scripts/package.py`，在 `dist/` 中获取合集包、十个普通独立 ZIP 和十个 `-workbuddy.zip` 根目录直接导入包。WorkBuddy 优先使用对应的直接导入包。
...
- “用短视频脚本与分镜助手，给员工写一条 45 秒的产品操作视频，一个人用手机拍，附分镜、口播和素材清单。”
- “用录屏转操作教程，把这个后台操作录屏整理成新人能照着做的图文手册，标注按钮，疑问单独列出，不上传原录屏。”
- “用 HTML 转 Word 与 PDF，把这几个 HTML 各转成 Word 和 PDF，保留正文、图片、表格和可点击链接，转换问题单独放报告。”
...
| 短视频脚本 | 写稿不需要额外工具；时间检查、CSV/粗剪 SRT 导出只需 Python 3.9+ 标准库，不生成或发布视频 |
| 录屏教程 | Python 3.9+、Pillow、FFmpeg/ffprobe；Word 另需 python-docx；步骤识别依赖宿主视觉/音频能力，无内置 OCR/ASR 或云上传 |
| HTML 转换 | Python 3.9+、lxml、Pillow、python-docx、Pandoc 3+；PDF 另需 LibreOffice、pypdf、中文字体。静态文档布局，不执行 JS 或自动抓远程资源 |
...
检查入口覆盖论文脚本回归、公众号原有测试、论文公众号交接与证据/配图门槛、小红书计划边界测试、短视频分镜导出、录屏提帧与教程导出、HTML 预检与文档转换及十技能结构/引用和目录搬迁检查。录屏测试使用模拟素材，不等于真实业务流程识别验收；缺可选依赖时会明确跳过相应测试。**未逐个完成五个客户端的原生导入与模型端到端测试**；Windows/Linux 原生运行也未实测。网络限流、图像生成、调度、OCR 与附件展示取决于实际环境，不能由技能格式兼容推断全部可用。
